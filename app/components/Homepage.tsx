...
const NitcheBox = ({children, iconUrl, heading}:NitcheBoxProps) => {
  return(
    <>
      <div className="border-gray-300 border bg-[#0f0f0f] rounded-3xl md:w-96 p-5 text-left areaContainer">
...
export function ProficientAreas() {
 return(
  <div className="text-white mt-[7rem]">
    ...
    <div className="flex justify-between gap-2.5 mt-4 md:gap-0 items-center pr-[1rem] md:text-5xl">
      <div className='text-white italic'>SKILL NICHE</div>
      <div className="w-[5rem] h-0.5 flex justify-center mt-2.5 bg-white md:w-[40rem]"></div>
      <div className='text-white italic'>SPECIALTIES</div>
    </div>
 
    <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-3 gap-y-10 pt-10 p-9">
...
const RecentProjectSection = () => {
 return(
  <div className="mt-[5rem]">
     <div className="text-4xl text-center text-white mt-8 md:text-[5rem]">
        <span className="font-bold text-white">RECENT</span> <span   className="italic text-white">PROJECTS</span> 
      </div>

    <section className="flex flex-col justify-center gap-2.5 items-center md:flex-row md:text-3xl md:mb-8">
       <div className="textSubHead">CASE STUDIES</div>
       <div className="md:w-2xs md:h-0.5 md:flex md:justify-center md:mt-3 md:bg-white hidden"></div>
       <div className="textSubHead">LIVE PROJECTS</div>
       <div className="md:w-2xs md:h-0.5 md:flex md:justify-center md:mt-3 md:bg-white hidden"></div>
       <div className="textSubHead">DESIGN PROCESS</div>
    </section>

    <section className="flex flex-col gap-[5rem] justify-center items-center md:flex-row">
      <div className="relative w-64 h-64">
        <Link href="/myQura">
          ...
          <Image src={coloredImage1} className="sm:opacity-100 absolute inset-0 w-max h-[20rem] object-cover lg:opacity-0 transition-opacity duration-300 hover:opacity-100" alt="image"/>
        </Link>
      </div>
      <div className="relative w-64 h-64">
        <Link href="/tantalizer">
          ...
          <Image src={coloredImage2} className="sm:opacity-100  absolute inset-0 w-[30rem] h-[20rem] object-cover lg:opacity-0 transition-opacity duration-300 hover:opacity-100" alt="image"/>
        </Link>
      </div>
      <div className="relative w-64 h-64">
        <Link href="/rock-realty">
          ...
          <Image src={coloredImage3} className="sm:opacity-100 absolute inset-0 w-[25rem] h-[20rem] object-cover lg:opacity-0 transition-opacity duration-300 hover:opacity-100" alt="image"/>
        </Link>
      </div>
    ...
    <div className="">
      ...
      <div className="pl-[2.5rem] pr-[0.5rem] font-poppins">
              <section className="relative">
                  ...
                  <div className="w-[18rem] absolute top-[10rem] left-[10%] right-[35%] font-figtree md:left-[20%] md:top-[25rem] lg:left-[25%] lg:top-[30rem] lg:w-[35rem]">
                    <h1 className="text-[5rem] lg:text-[9rem]">H O L A</h1>
                    <div className="flex text-[9px] justify-between font-brunoAce lg:text-15px">
                      <div>PRODUCT DESIGNER</div>
                      <div>LAWAL JIMOH</div>
                    </div>
                    <div className="flex items-center gap-2">
                    <h1 className="text-[5rem] lg:text-[9rem]">I A M </h1><Image src={menuIcon} className="h-[5rem] w-[5rem]" alt="image" />
                    </div>
                  </div>
              </section>

              <article className="mt-[10rem] md:mt-[1rem]">
                  <span className="text-3xl inline-block w-[15rem] md:text-[5rem] md:w-[35rem]">
                    GREAT DESIGN IS HARD, I MAKE IT EASY
                  </span>
                  <span className="text-2xl inline-block w-[100%] text-right mt-6 pr-[0.125rem] md:text-[2.5rem] md:w-[75%] md:ml-[15rem]">
              ...
              <div className="flex flex-col gap-[6rem] md:gap-[8rem]">
